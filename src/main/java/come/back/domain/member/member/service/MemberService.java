...
public class MemberService {
    ...
    public List<SettlementListResponseDto> getMySettlements(Long userId) {
        List<SettlementTransaction> transactions =
                settlementTransactionRepository.findByFromMember_Member_IdOrToMember_Member_Id(userId, userId);
        return transactions.stream().map(SettlementListResponseDto::from).toList();
    }
}
